...
public:
    ...
    virtual std::vector<std::string> getAllSections() const = 0;

    // 获取特定 Section 下的所有键值对
    virtual std::map<std::string, std::string> getSectionData(const std::string &section) const = 0;
    virtual bool setSectionData(const PluginInfo &info) = 0;
